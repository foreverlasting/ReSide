...
export function Settings({
  ...
  const revoke = useMutation({
    mutationFn: (serial: string) => {
      setRevoking(serial);
      return api.revokeCertificate(serial);
    },
    onSettled: () => {
      setRevoking(null);
      certs.refetch();
...
function AppleIdSection({
  ...
  const save = useMutation({
    mutationFn: () => api.setAppleCredentials(appleId, password, effectiveRemember),
    onSuccess: () => {
      setPassword("");
      onSaved();
    },
    ...
    <section className="rounded-xl border border-slate-200 bg-white dark:border-slate-800 dark:bg-slate-900">
      ...
      <div className="space-y-4 px-5 py-4">
        <div className="text-[12.5px] text-slate-500 dark:text-slate-400">
          {signedIn
            ? "Enter a different Apple ID below to switch accounts."
        ...
        <div className="flex justify-end">
          <Button
            size="sm"
            onClick={() => save.mutate()}
            disabled={!appleId || !password || save.isPending}
          >
